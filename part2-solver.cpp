...
int main(int argc, char* argv[]){
    ...
    std::cout<<"CHECK SUM\nInput port number with check sum bit puzzle: "; 
    int port_csum;
    std::cin >> port_csum;

    if ((port_csum<4000) | (4100<port_csum)){   
        perror("Port out of range [4000, 4100]");
        exit(0);
    }
	...
	udph->uh_dport= htons(port_csum);                                                    // Destination port
	...
	printf("\nSending group number to checksum puzzle port %d  (if it takes longer than 2sek, kill program (Ctrl+C) and retry)\n",port_csum);
    ...
    else
    {
		...
		while (checkSum(pseudogram, psize)!= hex_numb)
		{
			...
			if (index_str >= 4096){   // if bigger than size of payload then try again
    ...
    std::cout<<"EVIL BIT\nInput port number with evil bit puzzle: "; 
    int port_ebit;
    std::cin >> port_ebit;

    if ((port_ebit<4000) | (4100<port_ebit)){
    ...
    // set port 
	udph->uh_dport= htons(port_ebit);                                                    
	dest.sin_port =  htons(port_ebit); 
	...
	printf("\nSending group number to port %d  (if it takes longer than 2sek, kill program (Ctrl+C) and retry)\n",port_ebit);
   ...
   if (sendto (s_raw, datagram, iph->ip_len , 0, (struct sockaddr *) &dest, sizeof (dest)) < 0)
	...
	// Setting evil bit and sending again
	printf("Setting evil bit and sending... (if it takes longer than 2sek, kill program (Ctrl+C) and retry)\n");

	iph->ip_off = 0x80;   // setting evil bit
